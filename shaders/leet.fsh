...
void main() {
	...
	for (int i = 0; i < light_count; i++) {
		...
		if (lights[i].type == LIGHT_DIRECTIONAL) {
			vec3 light_dist = f_tbn * -lights[i].aux;
			diffuse = dot(normal, normalize(light_dist));
		} else if (lights[i].type == LIGHT_POINT) {
			vec3 light_dist = f_tbn * lights[i].position - f_tbn * f_pos;
			float factor = clamp(lights[i].aux.x / length(light_dist), 0.0, 1.0);
			diffuse = factor * dot(normal, normalize(light_dist));
		}
